...
```
docker run  --user itam --name r-console -v path_a_carpeta:/home/itam nanounanue/docker-ds /usr/bin/R
```

Lanzar la terminal de `python` con las librerías de cómputo científico:

```
docker run  --user itam --name ipython-console -v path_a_carpeta:/home/itam nanounanue/docker-ds /usr/bin/ipython
```

O un shell simple (en este caso `zsh`)

```
docker run --user itam  --name commandline -v path_a_carpeta:/home/itam nanounanue/docker-ds /bin/zsh
```
