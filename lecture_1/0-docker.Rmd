...
```
docker pull nanounanue/docker-ps
```

Creen un contenedor:

```
docker run -d -p 8787:8787 nanounanue/docker-ds
```
...
```
docker run --rm -it --user docker nanounanue/docker-ds /usr/bin/R
```

O un bash simple

```
docker run --rm -it --user docker nanounanue/docker-ds /bin/zsh
```
